...
const locales = {
  cn: {
    saturation: (s: string) =>
      `饱和度建议不低于${primaryMinSaturation}（当前值 ${s}）`,
    brightness: (b: string) =>
      `亮度建议不低于${primaryMinBrightness}（当前值 ${b}）`,
  },
  en: {
    saturation: (s: string) =>
...
const ColorPaletteTool: React.FC = () => {
  ...
  const [primaryColorInstance, setPrimaryColorInstance] =
  ...
  const colorValidation = React.useMemo<React.ReactNode>(() => {
    ...
    return (
      <span className="color-palette-picker-validation">{text.trim()}</span>
    );
